...
| 4 | Viewer: any page reachable by shelfmark, katalog record, or search hit; GWLB IIIF loaded directly, no rehosting; every line shows status + confidence on demand | **Built (D2); the "no rehosting" clause is diverged from by operator decision** (STATUS.md, Divergences), live 2026-09-23: the public site serves its own mirror of the GWLB delivery scans, PDM 1.0 — 236,779 images (395.6 GiB) + thumbnails on Cloudflare R2, verified complete before the switch — with every page linking to its original and the annotations still naming the GWLB URI as the source image. The direct-from-GWLB mode remains the code's default. OpenSeadragon, line overlay, status badges, confidence bands, provenance per line, EN/DE. Shelfmark/AA/katalog searches resolve through the index. | `src/leibniz/web/`, `web/images.py` |
