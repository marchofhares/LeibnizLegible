...
| 3 | Typo-tolerant full-text search over all machine transcriptions, grouped by piece with page hits, p95 < 500 ms, deep links into the viewer | **Built (D1); latency to be measured on the deployed index** (`leibniz index bench` reports p50/p95 over HTTP against the 500 ms bar; `deploy/README.md` §7). Meilisearch (typo tolerance) or SQLite FTS5 (prefix + orthography folding); results per page with work grouping in the viewer; deep links `/page/{page_id}`. Piece-level grouping is by work + katalog AA references, not yet by piece page-range. | `src/leibniz/search/`, `leibniz index build` |
...
3. **Operator:** deploy per `deploy/README.md` (serving copy of the store →
   `install.sh` on a small VPS → `leibniz index build --backend meili` →
   `leibniz index bench` for the p95 criterion), write the courtesy note to
   the GWLB before launch, then upload D1–D3 per the checklist.
